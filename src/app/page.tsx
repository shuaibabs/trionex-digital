...
import blogPostsImages from '@/lib/placeholder-images/blog-posts.json';
import testimonialsImages from '@/lib/placeholder-images/testiminials.json';
// import { clientsImages } from '@/lib/placeholder-images/clients.json';
import servicesImages from '@/lib/placeholder-images/services.json';
...
export default function Home() {
  ...
  React.useEffect(() => {
    ...
    onSelect(testimonialApi); // initial state
        ...
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-2 gap-8 md:grid-cols-4">
            <div className="text-center">
              ...
              <p className="mt-2 text-2xl sm:text-3xl font-bold font-headline">15+</p>
            ...
            <div className="text-center">
              ...
              <p className="mt-2 text-2xl sm:text-3xl font-bold font-headline">50+</p>
              <p className="text-muted-foreground text-sm sm:text-base">Projects Completed</p>
            </div>
            <div className="text-center">
              <LaptopMinimalCheck  className="mx-auto h-10 w-10 sm:h-12 sm:w-12 text-primary" />
              <p className="mt-2 text-2xl sm:text-3xl font-bold font-headline">100%</p>
              <p className="text-muted-foreground text-sm sm:text-base">Client Success Rate</p>
            </div>
            <div className="text-center">
        ...
        <div className="container mx-auto px-4">
          ...
          <div className="grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-4">
            {featuredServices.map((service, i) => {
              const serviceImage = service.imageIds.map(id => servicesImages.servicesImages.find(p => p.id === id)).filter(Boolean);
              return (
                <motion.div
                  key={service.id}
                  initial="hidden"
                ...
                >
                  ...
                  >
                    <LoadingLink href={`/services/${service.slug}`}>
                      ...
                      >
                        <CarouselContent>
                          {serviceImage.length > 0 ? serviceImage.map((img, index) => (
          ...
          >
            <div className="flex animate-scroll group-hover:[animation-play-state:paused]">
              {duplicatedClients.map((client, index) => {
                const clientImage = clientsImages.find(p => p.id === client.imageId);
                return (
          ...
          >
            <CarouselContent className="-ml-4">
              {testimonials.map((testimonial, index) => {
                const avatarImage = testimonialsImages.testimonialsImages.find(p => p.id === testimonial.avatarId);
        ...
        <div className="container mx-auto px-4">
          ...
          <div className="grid grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-3">
            {blogPosts.slice(0, 3).map((post, i) => {
              const blogImage = blogPostsImages.blogPostsImages.find(p => p.id === post.imageId);
              return (
                <motion.div
                  key={post.id}
                  initial="hidden"
